...
import { gql, InMemoryCache, FetchPolicy, WatchQueryFetchPolicy } from '@apollo/client/core';
...
export class QueryService {
  ...
  async saveLand(land: Land) {
    const response = await fetch(this.getURL('/api/land'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${this.localStorageService.get('token')}`
  ...
  updateGqlCache(query: any, variables: any, handler: (existing: any) => any) {
    const existing = this.cache?.readQuery({
      query: gql`${query}`,
      variables
    });
